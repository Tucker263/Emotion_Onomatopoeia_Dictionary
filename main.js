...
class EmotionObject{
    ...
    getOnomatopoeiaWords(){
        ...
        for(let i=0; i<this.onomatopoeia.length; i++){
            ...
            wordList.push(new Word(curr, dictionary[curr], pictureDictionary[curr]));
        }
        return wordList;
    }
    ...
    getHtmlContainerString(){
        let htmlContainer=
        `
        <div style="background:${this.color};" class="p-4" id="${this.emotion.toLowerCase()}">
            <h4 style="color:white;">${this.emotion.toUpperCase()}</h4>
        ...
        for(let i=0; i<wordList.length; i++){
            htmlContainer+=
            `
            <div style="background:white;" class="p-2 m-3 col-sm-5 row align-items-center">
                <div class="col-6">
                    <h5>${wordList[i].word}</h5>
                    <p>${wordList[i].defintion}</p>
                </div>
                <div class="col-6">
                    <img src="${wordList[i].pictureUrl}" class="img-fluid">
                </div>
            </div>
            `;
        }
...
class HelperFunctions{
    ...
    static getCategoryContainerString(){
        let categoryCotainer=
        `
        <div class="bg-light p-4 d-flex justify-content-center">
        <div class="col-8">
            <h2 class="ml-sm-2 ml-lg-5">Category</h2>
        ...
        for(let i=0; i<emotions.length; i++){
            categoryCotainer+=
            `
            <div style="background:${emotions[i].color};" class="p-3 m-2 col-sm-5 col-lg-3">
                <h4 class="text-center"><a href="#${emotions[i].emotion.toLowerCase()}" style="color:white">${emotions[i].emotion.toUpperCase()}</a></h4>
                <img src="${pictureDictionary[emotions[i].emotion.toLowerCase()]}" class="img-fluid">  
            </div>
            `;
        }
        ...
        return categoryCotainer;
    }

    static setHtmlEmotions(){
        let targetDiv = document.getElementById("target");
        //前半部分の感情のカテゴリーを作成
        targetDiv.innerHTML = HelperFunctions.getCategoryContainerString();
        //後半部分の感情の各セクションを作成
        for(let i=0; i<emotions.length; i++){
            targetDiv.innerHTML+=emotions[i].getHtmlContainerString();
        }
    }
...
const emotions = [
    ...
    //ここから追加部分
    new EmotionObject("exciting", "feeling or showing pleasure or contentment.", "pink", ["bling","chatter","chant","giggle"]),
    new EmotionObject("hurt", "feeling or showing strong annoyance, displeasure, or hostility; full of anger.", "burlywood", ["bark","grunt", "roar","whack","smack","hiss"])
];


//htmlをセット
HelperFunctions.setHtmlEmotions();